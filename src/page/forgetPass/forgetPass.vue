<template>
  <div>
    ...
    <section class="c_login">
      <div class="c_form">
        <div class="c_line">
          <input v-model="phoneNumber" maxlength="11" placeholder="请输入你的手机号码"/>
        ...
        <div class="c_line">
          <input v-model="codeNumber" maxlength="4" placeholder="请输入验证码"/>
        </div>
        <div class="c_line">
          <input v-if="!showPassword" type="password" placeholder="请输入您的新密码" v-model="password">
          <input v-else type="text" placeholder="密码" v-model="password">
          <div class="button_switch" :class="{change_to_text: showPassword}">
            <div class="circel_button" :class="{trans_to_right: showPassword}" @click="changePassWordType"></div>
          </div>
        </div>
        <div class="c_btn" @click="userMobileFindPass">确认</div>
      </div>
    </section>
  </div>
</template>
...
<script>
  ...
  import {mobileCode, userMobileFindPass} from '../../service/getData';
  ...
  export default {
    ...
    methods: {
      ...
      async userMobileFindPass() {
        ...
        await userMobileFindPass(this.phoneNumber, this.password, this.codeNumber).then(function (res) {
          this.toastContent = res.msg;
          this.showToastPlugin();
          console.log(res);
          if (res.code == 1) {
