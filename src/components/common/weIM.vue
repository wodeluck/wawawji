  ...
  <div>
    <div class="fixScroll">
      <ul class="messages" v-chat-scroll>
        <li v-for="(item,index) in msgList2" :key="index">{{item}}</li>
      </ul>
    </div>
  </div>
</template>
...
<script>
  ...
  import {escape2Html} from "../../config/mUtils";
  ...
  export default {
    ...
    watch: {
      beginSendMsg: function () {
        this.sendCommend(this.avChatRoomId,JSON.stringify({type:this.msgType,user_id:this.userInfo.id,avatar:this.userInfo.avatar,user_nicename:this.userInfo.user_nicename,message:this.sendMsgText,game_id:this.gameId,room_id:this.roomId}),function (res) {
          console.log(res);
        });
      },
    ...
    props: [
      'tim_uid', 'tim_sig', 'sendMsgText', 'beginSendMsg', 'avChatRoomId', 'commendUserName', 'exitIM', 'msgType','roomId','gameId','notice','reservationRandomNum'
    ],
    mounted() {
...
//处理消息（私聊(包括普通消息和全员推送消息)，普通群(非直播聊天室)消息）
        ...
        let loginInfo = {
          ...
          'identifier': _this.tim_uid, //当前用户ID,必须是否字符串类型，选填
        ...
        try {
          ...
          if (msg) {
            ...
            if (_msg.indexOf('{') == 0) {
              ...
              switch (_msg.type) {
                case 1://聊天消息
                  this.msgList2.push(_msg.message);
                  setTimeout(function () {
                    this.$nextTick(() => {
                ...
                case 2://开始游戏
                  ...
                  break;
                case 3://进入房间
                  this.$emit('listenToEnterInRoom',{user_nickname:_msg.user_nicename,avatar:_msg.avatar,user_id:_msg.user_id});
                  break;
                case 4://离开房间
