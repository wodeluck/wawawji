...
function sdkLogin(identifier, userSig) {
  console.log('login');
  //web sdk 登录
...
sendGroupMsg('0', JSON.stingify({ 'type':"2", user_id: 111, }), function(){

})
function sendGroupMsg(groupid, msgtosend, cbfunc) {
  var _selToID = groupid || selToID;
  if (!selSess) {
    selSess = new webim.Session(selType, _selToID, _selToID, selSessHeadUrl, Math.round(new Date().getTime() / 1000));
  }
  var isSend = true; //是否为自己发送
  var seq = -1; //消息序列，-1表示sdk自动生成，用于去重
  var random = Math.round(Math.random() * 4294967296); //消息随机数，用于去重
...
function quitBigGroup(_avChatRoomId) {
  var options = {
    'GroupId': _avChatRoomId || avChatRoomId //群id
  };
  webim.quitBigGroup(
    options,
    function(resp) {
